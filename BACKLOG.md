...
- [2026-09-04] [INVEST] First clean-checkout CI/CD deploy revealed `admin.api-key` was missing entirely from `/home/ubuntu/application.properties` on the server (masked for months by locally-built images baking in a dev machine's gitignored properties file). Fixed for that one key, but the same class of drift could exist for keys that already exist on the server with a stale/dev *value* (not missing key) — e.g. `aws.iot.clientId` — since only presence was audited, not correctness of values. Worth a one-time full read-through comparison of server `application.properties` values against what production actually needs.

## Done

(archive items here as they're closed; prune older than ~3 months)

- [2026-09-04] [DOC] `ECR-Deployment.md` rewritten to match actual prod setup (`/home/ubuntu/` paths, real `docker-compose.prod.yml`, no stale `pid: host`/`cap_add`/`tunneluser` volume/`config/` mount) and to document the new GitHub Actions CI/CD pipeline (OIDC role, required secrets/variables, the server `application.properties` completeness requirement, and manual rollback).
